...
let extname = () =>
  Alcotest.(check(string))("Path.extname", ".html", Path.extname("foo/bar/lol.html"));
...
let formatWithRootNameExt = () =>
  Alcotest.(check(string))(
    ...
    "/file.txt",
    Path.(format({dir: None, root: Some("/"), base: None, name: Some("file"), ext: Some(".txt")}))
  );
...
let resolvePath0 = () =>
  Alcotest.(check(string))("Path.resolve 0", "/foo/bar/baz", Path.resolve(["/foo/bar", "./baz"]));
